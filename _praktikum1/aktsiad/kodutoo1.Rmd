...
Andmestikus on näidatud 504 ettevõtte (kõik need ettevõtted, mis on kuulsas S&P 500 indeksis) kohta järgmised tunnused:
...
Laadi alla andmestik [aktsiad.csv](/2016/data/aktsiad.csv) ning loe andmestik töökeskonda.
...
* Arvuta iga sektori kohta
  ...
  Näpunäide 2: Summade ja keskmiste arvutamisel ära võta arvesse puuduvaid väärtusi (puuduvate väärtuse välja jätmiseks võib funktsioonide `sum` ja `mean` argumendi `na.rm` väärtuseks seada `FALSE`).
